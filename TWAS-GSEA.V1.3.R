...
if(opt$allow_duplicate_ID == F){
	...
	TWAS<-TWAS[order(abs(TWAS$TWAS.Z), decreasing = TRUE),]
	if(is.na(opt$use_alt_id)){
...
# I want all genes to be tested by the mixed models, so I'll assign the whole "Linear_Results_temp" as
# the "gene_sets_clean_forMLM" tab

# 2019_05-25: for the enrichment analysis on the meta-analysis results, I don't need to do
# a meta-analysis of the gene-set enrichment from each dataset, so there is no need to generate
# mixed model competitive results for all gene-sets; actually, now that the expression matrix
# that will be used to model the correlation between genes due to linkage-disequilibrium is much
# larger, the competitive analysis would take much longer, so it's probably much better to perform
# it only on those gene-sets that pass certain p-value threshold in the standard linear model

# gene_sets_clean_forMLM <- Linear_Results_temp$GeneSet
...
#		sink(file = paste(opt$output,'.log',sep=''), append = T)
		...
		cor_block_all<-foreach(i=unique(TWAS_GS_Mem_clean$Block), .combine=bdiag_withNames) %dopar% {
			if(sum(TWAS_GS_Mem_clean$Block == i) == 1){
				...
				cor_block<-abs(WGCNA::cor(as.matrix(GeneX_all[(names(GeneX_all) %in% TWAS_GS_Mem_clean$FILE[TWAS_GS_Mem_clean$Block == i])]), method='pearson', use = "pairwise.complete.obs"))
	...
	if(opt$competitive == T){
		...
		Results_Comp<-foreach(i=1:length(gene_sets_clean_forMLM), .combine=rbind) %dopar% {
			if(opt$covar != 'none'){
				...
				mod_alt<-mod
				mod_X<-mod@pp$X
				mod_alt@pp <- merPredD(X=cbind(mod@pp$X,TWAS_GS_Mem_clean[,which(!is.na(match(colnames(TWAS_GS_Mem_clean), gene_sets_clean_forMLM[i])))]), Zt=mod@pp$Zt, Lambdat=mod@pp$Lambdat, Lind=mod@pp$Lind, theta=mod@pp$theta, n=nrow(mod@pp$X))
				mod2<-refit(mod_alt, TWAS_GS_Mem_clean$ZSCORE)
			}
...
#		sink(file = paste(opt$output,'.linear.sig.txt',sep=''), append = F)
		...
		for(i in 1:dim(Results_sig)[1]){
			...
			TWAS_SigSet<-TWAS_SigSet[c('FILE','ID','CHR','P0','P1','TWAS.Z','TWAS.P','ZSCORE')]
